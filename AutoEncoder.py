...
class AutoEncoder(NeuralNetworkBase):
    def __init__(self, input_size, hidden_layers, num_classes, batch_norm):
        ...
        # Encoder layers
        encoder_layers, decoder_layers = self.create_layers(input_size, hidden_layers, batch_norm)      
        self.encoder = nn.Sequential(*encoder_layers)
        # Latenter Raum
        self.latent = nn.Linear(hidden_layers[-1], hidden_layers[-1])
        # Decoder layers
    ...
    def create_layers(self, input_size, hidden_layers, batch_norm):
        
        encoder_layers = []
        encoder_layers.append(nn.Linear(input_size, hidden_layers[0]))
        
        # test batchnorm
        if batch_norm == True:
            encoder_layers.append(nn.BatchNorm1d(hidden_layers[0]))
        
        encoder_layers.append(nn.ReLU())
        for i in range(len(hidden_layers) - 1):
            encoder_layers.append(nn.Linear(hidden_layers[i], hidden_layers[i + 1]))
            if batch_norm:
                encoder_layers.append(nn.BatchNorm1d(hidden_layers[i + 1])) 
        ...
        for i in range(len(hidden_layers) - 1, 0, -1):
            ...
            # test batchnorm
            if batch_norm:
                decoder_layers.append(nn.BatchNorm1d(hidden_layers[i - 1])) 
    ...
    def forward(self, x):
        ...
        return encoded, decoded, classifier
    ...
    def test_model(self, test_loader, mode):
        ...
        if mode == "decode":
            ...
            if all_reconstructions and all_originals:
                ...
                mse = mean_squared_error(all_originals, all_reconstructions)
    ...
    def train_model(self, data_loader, val_loader, num_epochs, input_size, optimizer, criterion, validation, mode):
                ...
                if validation == True:
                    ...
                    with torch.no_grad():
                        for val_data, val_labels in val_loader:
                            ...
                            val_loss += criterion(val_classifications, val_labels).item()
